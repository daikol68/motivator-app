...
import de.daikol.acclaim.adapter.ChallengeAdapter;
import de.daikol.acclaim.adapter.ChallengeTouchHelper;
import de.daikol.acclaim.adapter.MessageAdapter;
import de.daikol.acclaim.adapter.MessageTouchHelper;
import de.daikol.acclaim.application.Challenger;
import de.daikol.acclaim.fragment.LeaveCompetitionDialog;
...
import de.daikol.acclaim.listener.OnListUnreadComplete;
import de.daikol.acclaim.model.Competition;
import de.daikol.acclaim.model.Message;
import de.daikol.acclaim.model.user.User;
import de.daikol.acclaim.tasks.DeclineCompetitionTask;
import de.daikol.acclaim.tasks.ListChallengesTask;
import de.daikol.acclaim.tasks.ListUnreadMessagesTask;
import de.daikol.acclaim.tasks.ReadMessageTask;
...
public class MainActivity extends AppCompatActivity implements
    ...
    private void update() {
        ...
        if (viewSwitch == R.id.nav_messages) {
            showLoader();
            new ListUnreadMessagesTask(challenger, this).execute();
        } else if (viewSwitch == R.id.nav_challenges) {
            showLoader();
            new ListChallengesTask(challenger, this).execute();
        }
    }
    ...
    public void onSwipedMessage(RecyclerView.ViewHolder viewHolder, int direction, int position) {
        ...
        new ReadMessageTask(Long.valueOf(holder.messageId.getText().toString()), challenger).execute();
    ...
    public void onListUnreadCompleted(List<Message> messages) {
        messageAdapter = new MessageAdapter(messages, challenger, getApplicationContext(), getFragmentManager());
        recycler.setAdapter(messageAdapter);
        recycler.invalidate();
        hideLoader();
    }
    ...
    public void onListChallengesCompleted(List<Competition> competitions) {
        challengeAdapter = new ChallengeAdapter(competitions, challenger, getApplicationContext(), getFragmentManager());
        recycler.setAdapter(challengeAdapter);
        recycler.invalidate();
        hideLoader();
    }
    ...
    public void onLeaveCompetitionPositive(Competition competition) {
        ...
        new DeclineCompetitionTask(competition.getId(), challenger).execute();
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        message.setReceiver(challenger.getUser());
